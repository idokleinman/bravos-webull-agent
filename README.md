...
| **2 Injection** | `aws lambda invoke … --payload '{"test_signal":"Aggressive","message_id":"uniq-123"}'` or `sam local invoke -e events/test_aggressive.json` | **Yes** (IAM-trusted, not a spoofable email). Use a **unique** `message_id` each run. |
